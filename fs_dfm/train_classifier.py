#
# Fine-tune FS-DFM for IMDB Sentiment Classification (Generative Approach)
#
# Three loss modes:
...
def train_step_cross_entropy(model, x_1, batch, source_distribution, path, loss_fn, vocab_size, device, amp_dtype, use_amp):
...
def train_step_generalized_kl(model, x_1, batch, source_distribution, path, loss_fn, vocab_size, device, amp_dtype, use_amp):
...
def train_step_distillation(
    ...
    distill_th, can_apply_dt, device, amp_dtype, use_amp,
):
...
def train(args):
    ...
    # --- Weights & Biases ---
    use_wandb = args.enable_wandb and wandb is not None
    if args.enable_wandb and wandb is None:
    ...
    if args.loss_mode in ("cross_entropy", "generalized_kl"):
        ...
        if args.checkpoint_path and os.path.exists(args.checkpoint_path):
            # Load into teacher
            load_pretrained_weights(args.checkpoint_path, teacher_model, device)
        ...
        # Solver (needed for RK-4 estimation)
        mask_token = vocab_size  # mask source adds extra token
        ...
        # For distillation, model reference = student (trained), teacher is frozen
        model = student_model
    ...
    while step < args.num_iters:
        ...
        # Move full batch to device for label metrics
        ...
        if args.loss_mode == "cross_entropy":
            loss, metrics = train_step_cross_entropy(
                model, x_1, batch_device, source_distribution, path, loss_fn, vocab_size, device, amp_dtype, use_amp
            )
        elif args.loss_mode == "generalized_kl":
        ...
        grad_norm = torch.nn.utils.clip_grad_norm_(trainable_params, args.grad_clip if args.grad_clip > 0 else float("inf"))
        ...
        # EMA update (distillation mode only)
        if args.loss_mode == "distillation" and step % args.ema_freq == 0:
            ema_update(student_ema_model, student_model, args.ema_decay)
        ...
        if step % args.save_freq == 0 or step == args.num_iters:
            ...
            if args.loss_mode == "distillation":
                ...
                save_dict["student_model"] = student_model.state_dict()
                save_dict["student_ema_model"] = student_ema_model.state_dict()
                save_dict["optimizer"] = optimizer.state_dict()
            else:
                save_dict["teacher_model"] = model.state_dict()
                save_dict["optimizer"] = optimizer.state_dict()
